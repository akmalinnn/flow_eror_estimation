...
# 🔹 Konfigurasi Model
# ===========================
model_path = "resnet18_epoch1000.pth"  # Path model yang sudah dilatih
of_dir = "flow/06_0205"  # Folder optical flow
...
model = timm.create_model("resnet18", pretrained=False, num_classes=num_c)

# Ubah input channel agar bisa menerima optical flow (2 channel)
model.conv1 = torch.nn.Conv2d(in_c, model.conv1.out_channels, 
...
def predict_speed_batch(npy_files):
    ...
    with torch.no_grad():
        ...
        pred_speeds = pred_norms * (max_speed - min_speed) + min_speed  # Denormalisasi
