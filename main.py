...
sys.path.append(os.path.abspath("Sernet_former"))
...
def extract_frames(video_path, output_folder, width=640, height=360, fps=24):
    os.makedirs(output_folder, exist_ok=True)
    video_name = os.path.splitext(os.path.basename(video_path))[0]
    ffmpeg_cmd = [
        "ffmpeg", "-i", video_path,
        "-vf", f"fps={fps},scale={width}:{height}",
        os.path.join(output_folder, f"{video_name}_frame_%05d.jpg")
    ]
    subprocess.run(ffmpeg_cmd, check=True)
    print(f"Extracted frames for {video_name} at {fps} fps in {output_folder}")
...
def subtract_npy_files(input_folder, output_folder, frame_folder):
    ...
    for i in range(len(npy_files) - 1):
        ...
        img_output_path = os.path.join(output_folder, f"{output_filename}.jpg")
